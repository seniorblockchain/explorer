...
import { ScrollbarGeometry, ScrollbarPosition } from '@blockcore/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[blockcoreScrollbar]',
    exportAs: 'blockcoreScrollbar'
})
export class blockcoreScrollbarDirective implements OnChanges, OnInit, OnDestroy
{
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_blockcoreScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() blockcoreScrollbar: boolean = true;
    @Input() blockcoreScrollbarOptions: PerfectScrollbar.Options;
    ...
    {
        // Enabled
        if ( 'blockcoreScrollbar' in changes )
        {
            // Interpret empty string as 'true'
            this.blockcoreScrollbar = coerceBooleanProperty(changes.blockcoreScrollbar.currentValue);

            // If enabled, init the directive
            if ( this.blockcoreScrollbar )
            {
                this._init();
            }
        ...
        // Scrollbar options
        if ( 'blockcoreScrollbarOptions' in changes )
        {
            // Merge the options
            this._options = merge({}, this._options, changes.blockcoreScrollbarOptions.currentValue);
    ...
    isEnabled(): boolean
    {
        return this.blockcoreScrollbar;
    }
