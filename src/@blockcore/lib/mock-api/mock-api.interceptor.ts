...
import { blockcoreMockApiService } from '@blockcore/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root'
})
export class blockcoreMockApiInterceptor implements HttpInterceptor
{
    /**
     * Constructor
     */
    constructor(
        @Inject(blockcore_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _blockcoreMockApiService: blockcoreMockApiService
    )
    {
    }
    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>>
    {
        ...
        const {
                  handler,
                  urlParams
              } = this._blockcoreMockApiService.findHandler(request.method.toUpperCase(), request.url);
