...
const generateVariableColors = (theme) =>
{
    ...
    }) =>
    {
        if ( opacityValue )
        {
            return `rgba(var(--blockcore-${name}-rgb), ${opacityValue})`;
        }
        if ( opacityVariable )
        {
            return `rgba(var(--blockcore-${name}-rgb), var(${opacityVariable}, 1))`;
        }
        return `rgb(var(--blockcore-${name}-rgb))`;
    };
...
// -----------------------------------------------------------------------------------------------------
// @ blockcore TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
const theming = plugin.withOptions((options) => ({
    ...
    }) =>
    {
        ...
        const mapVariableColors = _.fromPairs(_.map(options.themes, (theme, themeName) => [
            ...
            ))), (value, key) => [[`--blockcore-${e(key)}`, value], [`--blockcore-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
        ]));
        ...
        const schemeCustomProps = _.map(['light', 'dark'], (colorScheme) =>
        {
            ...
            const background = theme(`blockcore.customProps.background.${colorScheme}`);
            const foreground = theme(`blockcore.customProps.foreground.${colorScheme}`);
            ...
            return {
                [(isDark ? darkSchemeSelectors : lightSchemeSelectors)]: {
                    ...
                    ..._.fromPairs(_.flatten(_.map(background, (value, key) => [[`--blockcore-${e(key)}`, value], [`--blockcore-${e(key)}-rgb`, chroma(value).rgb().join(',')]]))),
                    ..._.fromPairs(_.flatten(_.map(foreground, (value, key) => [[`--blockcore-${e(key)}`, value], [`--blockcore-${e(key)}-rgb`, chroma(value).rgb().join(',')]])))
                }
            };
        });
