...
import { blockcoreNavigationItem, blockcoreVerticalNavigationAppearance, blockcoreVerticalNavigationMode, blockcoreVerticalNavigationPosition } from '@blockcore/components/navigation/navigation.types';
...
@Component({
    selector       : 'blockcore-vertical-navigation',
    templateUrl    : './vertical.component.html',
    styleUrls      : ['./vertical.component.scss'],
    animations     : blockcoreAnimations,
    encapsulation  : ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs       : 'blockcoreVerticalNavigation'
})
// eslint-disable-next-line @typescript-eslint/naming-convention
export class blockcoreVerticalNavigationComponent implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: blockcoreVerticalNavigationAppearance = 'default';
    ...
    @Output() readonly appearanceChanged: EventEmitter<blockcoreVerticalNavigationAppearance> = new EventEmitter<blockcoreVerticalNavigationAppearance>();
    @Output() readonly modeChanged: EventEmitter<blockcoreVerticalNavigationMode> = new EventEmitter<blockcoreVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> = new EventEmitter<boolean>();
    @Output() readonly positionChanged: EventEmitter<blockcoreVerticalNavigationPosition> = new EventEmitter<blockcoreVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<blockcoreNavigationItem> = new ReplaySubject<blockcoreNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<blockcoreNavigationItem> = new ReplaySubject<blockcoreNavigationItem>(1);
    ...
    {
        return {
            'blockcore-vertical-navigation-animations-enabled'             : this._animationsEnabled,
            [`blockcore-vertical-navigation-appearance-${this.appearance}`]: true,
            'blockcore-vertical-navigation-hover'                          : this._hovered,
            'blockcore-vertical-navigation-inner'                          : this.inner,
            'blockcore-vertical-navigation-mode-over'                      : this.mode === 'over',
            'blockcore-vertical-navigation-mode-side'                      : this.mode === 'side',
            'blockcore-vertical-navigation-opened'                         : this.opened,
            'blockcore-vertical-navigation-position-left'                  : this.position === 'left',
            'blockcore-vertical-navigation-position-right'                 : this.position === 'right'
        };
    }
    ...
    /**
     * Setter for blockcoreScrollbarDirectives
     */
    @ViewChildren(blockcoreScrollbarDirective)
    set blockcoreScrollbarDirectives(blockcoreScrollbarDirectives: QueryList<blockcoreScrollbarDirective>)
    {
        // Store the directives
        this._blockcoreScrollbarDirectives = blockcoreScrollbarDirectives;

        // Return if there are no directives
        if ( blockcoreScrollbarDirectives.length === 0 )
        {
            return;
        }

        // Unsubscribe the previous subscriptions
        if ( this._blockcoreScrollbarDirectivesSubscription )
        {
            this._blockcoreScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._blockcoreScrollbarDirectivesSubscription =
            merge(
                ...
                .subscribe(() => {
                    ...
                    blockcoreScrollbarDirectives.forEach((blockcoreScrollbarDirective) => {
                        blockcoreScrollbarDirective.update();
                    });
                });
    }
    ...
    ngOnInit(): void
    {
        ...
        this._blockcoreNavigationService.registerComponent(this.name, this);
    ...
    ngAfterViewInit(): void
    {
        setTimeout(() => {
            ...
            if ( !this._navigationContentEl.nativeElement.classList.contains('ps') )
            {
                ...
                const activeItem = this._navigationContentEl.nativeElement.querySelector('.blockcore-vertical-navigation-item-active');
            ...
            else
            {
                ...
                // eslint-disable-next-line @typescript-eslint/no-shadow
                this._blockcoreScrollbarDirectives.forEach((blockcoreScrollbarDirective) => {

                    // Skip if not enabled
                    if ( !blockcoreScrollbarDirective.isEnabled() )
                    {
                        return;
                    }

                    // Scroll to the active element
                    blockcoreScrollbarDirective.scrollToElement('.blockcore-vertical-navigation-item-active', -120, true);
                });
            }
        });
    }
    ...
    ngOnDestroy(): void
    {
        ...
        this._blockcoreNavigationService.deregisterComponent(this.name);
     ...
     * @param item
     */
    openAside(item: blockcoreNavigationItem): void
    {
     ...
     * @param item
     */
    toggleAside(item: blockcoreNavigationItem): void
    {
        // Toggle
    ...
    private _showOverlay(): void
    {
        ...
        this._overlay.classList.add('blockcore-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if ( this.transparentOverlay )
        {
            this._overlay.classList.add('blockcore-vertical-navigation-overlay-transparent');
        }
    ...
    private _showAsideOverlay(): void
    {
        ...
        this._asideOverlay.classList.add('blockcore-vertical-navigation-aside-overlay');
